...
export class CityService {
    public async getCityListService(query: ICityFilter, limit: number, skip: number) {
        try {
            let operationPromise: any;

            const cityRepository = new CityRepository();
            operationPromise = await cityRepository.getCityListRepository(query, limit, skip);
            if (!operationPromise) return ({ msg: 'Erro getCityListService', status: 0 });
