...
export class RoleRepository {
  public async getRoleListRepository(query: object) {
    try {
      ...
      if (!operationPromise || operationPromise.length <= 0)
        return { msg: "Não existe roles cadastradas", status: 0 }
  ...
  public async insertRoleRepository(role: IRole) {
    try {
      ...
      operationPromise = await RolesSchema.find({
        $and: [{ name_role: role.name_role }, { code_role: role.code_role }],
      })
      if (!operationPromise || operationPromise.length >= 1)
        return { msg: "Já existe uma role com esse nome e code", status: 0 }
  ...
  public async updateRoleRepository(id: string, role: IRole) {
    try {
      ...
      operationPromise = await RolesSchema.find({ _id: id })
      if (!operationPromise || operationPromise.length <= 0)
        return { msg: `Não existe role com esse id`, status: 0 }

      operationPromise = await RolesSchema.findOneAndUpdate(
        { _id: id },
        {
      ...
      if (!operationPromise)
        return { msg: `Erro ao buscar role atualizada`, status: 0 }
