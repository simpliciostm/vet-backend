...
import { ICity, ICityFilter } from "../../interface/City";

export class CityRepository {
    public async getCityListRepository(query: ICityFilter, limit: number, skip: number) {
        try {
            ...
            if (filter) {
                ...
                if (!operationPromise || operationPromise.length <= 0) return ({ msg: 'Não existe cidade com esse filtro', status: 0 });
                cads = operationPromise ? operationPromise : null;
            } else {
                operationPromise = await City.find({}).skip(skip).limit(limit);
                if (!operationPromise || operationPromise.length <= 0) return ({ msg: 'Não existe cidade cadastrado', status: 0 });
                cads = operationPromise ? operationPromise : null;
            }
            ...
            return ({ msg: 'Cidades cadastradas', status: 1, data: cads, columns: columns, total: totalCads });
        } catch (err) {
            return ({ msg: err });
        }
    }
    ...
    public async insertCityRepository(city: ICity) {
        try {
            ...
            operationPromise = await City.find(filter);
            if (!operationPromise || operationPromise.length >= 1) return ({ msg: 'Já existe uma Cidade com esse nome', status: 0 });

            operationPromise = await City.create(city);
            if (!operationPromise) return ({ msg: `Erro ao criar Cidade`, status: 0 });

            return ({ msg: `Cidade criada com sucesso`, status: 1, data: operationPromise });
        } catch (err) {
            return ({ msg: err });
        }
    }
    ...
    public async deleteCityRepository(id: string) {
        try {
            ...
            if (!operationPromise || operationPromise <= 0) return ({ msg: 'Não existe uma Cidade com esse id', status: 0 });

            operationPromise = await City.findOneAndDelete({ _id: id });
            if (!operationPromise) return ({ msg: `Erro ao excluir Cidade`, status: 0 });

            return ({ msg: 'Cidade deleteada com sucesso', status: 1, data: operationPromise });
        } catch (err) {
            return ({ msg: err });
        }
    }
    ...
    public async updateCityRepository(id: string, city: ICity) {
        try {
            ...
            operationPromise = await City.find({ _id: id });
            if (!operationPromise || operationPromise.length <= 0) return ({ msg: `Não existe Cidade com esse id`, status: 0 });
            ...
            if (!operationPromise) return ({ msg: `Erro ao buscar Cidade atualizada`, status: 0 });

            return ({ msg: `Cidade atualizada com sucesso`, status: 1, data: operationPromise });
        } catch (err) {
            return ({ msg: err });
        }
    }
