...
export class PermissionRepository {
  public async getPermissionListRepository(query: object) {
    try {
      ...
      if (!operationPromise || operationPromise.length <= 0)
        return { msg: "Não existe permissions cadastradas", status: 0 }

      return {
        msg: "Permissions cadastrados",
        status: 1,
  ...
  public async insertPermissionRepository(permission: IPermission) {
    try {
      if (!permission)
        return { msg: "Permission params undefined or null", status: 0 }
      ...
      if (!operationPromise || operationPromise.length >= 1)
        return { msg: "Já existe uma Permissão com esse nome", status: 0 }

      operationPromise = await Permission.create(permission)
      if (!operationPromise)
        return { msg: `Erro ao criar Permissions`, status: 0 }
  ...
  public async deletePermissionRepository(id: string) {
    try {
      ...
      if (!operationPromise || operationPromise <= 0)
        return { msg: "Não existe uma Permission com esse id", status: 0 }

      operationPromise = await Permission.findOneAndDelete({ _id: id })
      if (!operationPromise)
        return { msg: `Erro ao excluir Permission`, status: 0 }

      return {
        msg: "Permissão deleteada com sucesso",
        status: 1,
        data: operationPromise,
      }
    } catch (err) {
  ...
  public async updatePermissionRepository(id: string, permission: IPermission) {
    try {
      ...
      operationPromise = await Permission.find({ _id: id })
      if (!operationPromise || operationPromise.length <= 0)
        return { msg: `Não existe Permission com esse id`, status: 0 }

      operationPromise = await Permission.findOneAndUpdate(
        { _id: id },
        {
          name_Permission: permission.name_permission,
      ...
      if (!operationPromise)
        return { msg: `Erro ao buscar Permission atualizada`, status: 0 }
